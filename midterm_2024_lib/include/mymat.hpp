...
// Modified by Beomsoo Shin (for midterm)
...
class MatBase
{
    ...
    protected: // accessible only for class and subclass member functions
        ...
        double m_elements[max_size][max_size];
...
// 4d vector class (subclass of MatBase)
// Inherits member functions and variables from MatBase
class Vec4 : public MatBase
{
public:
    Vec4(); // constructor
    Vec4(double e0, double e1, double e2, double e3); // constructor with values
    Vec4(const MatBase& base) : MatBase(base) {  } // cast to MatBase

    double dot(const Vec4 &vec); // dot product
    double squarednorm(); // squared norm
    void display(); // vector visualization
};
